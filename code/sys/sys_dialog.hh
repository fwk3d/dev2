...
SetWindowPos(hwndInputBox, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
...

    {
        ...
        myinp.show(title, body ? body : "", result, 2048);
        myinp.destroy();
...
char* prompt(const char *title, const char *body, const char *defaults ) {
    ...
    char *ret = (tinyfd_assumeGraphicDisplay = 1, tinyfd_inputBox(title, body, defaults));
#else
    char *ret = osdialog_prompt(0/*OSDIALOG_INFO*/, body, defaults);
#endif
    return ret;
}
