...
static void SDLCALL playback_device_callback(void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount)
{
    ...
    SDL_PutAudioStreamData(device->sdlstream, data, additional_amount);
...
//      desired.userdata = device;
        device->sdlstream = SDL_OpenAudioDeviceStream(devices[use_device], &desired, playback_device_callback, device);
